 ...
 * Records an accountable admin decision. Approval does not publish;
 * /activate performs the final publication gate.
 */
export async function POST(
  ...
  if (project.status === ListingStatus.PUBLISHED) {
    return NextResponse.json(
      {
        error: "AlreadyPublished",
        message: "Unpublish the project before changing its audit decision",
      },
      { status: 409 },
    );
  }
